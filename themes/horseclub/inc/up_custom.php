...
function horseclub_detect_mobile()
{
...
///Page Navigation
	function horseclub_wp_pagenavi() {
...
function up_excerpt($limit) {
      $excerpt = explode(' ', get_the_excerpt(), $limit);
      if (count($excerpt)>=$limit) {
        array_pop($excerpt);
        $excerpt = implode(" ",$excerpt).'...<div class="more_post"><a class="post_more" href="' . get_permalink() . '">' . esc_html__('Continued', 'horseclub') . '</a></div>';
      } else {
	...
	// Ecerpt Length by character number
function get_excerpt_up( $count ) {
global $post;	
...
$excerpt = '<p>'.$excerpt.'... <div class="more_post"><a class="post_more" href="' . get_permalink() . '">' . esc_html__('Continued', 'horseclub') . '</a></div>';
return $excerpt;
}	

function horseclub_content($limit) {
...
function horseclub_get_adjacent_post_plus($r, $previous = true ) {
  global $post, $wpdb;
 ...
 */
function horseclub_previous_post_link_plus($args = '') {
  return horseclub_adjacent_post_link_plus($args, '&laquo; %link', true);
}
 ...
 */
function horseclub_next_post_link_plus($args = '') {
  return horseclub_adjacent_post_link_plus($args, '%link &raquo;', false);
}
 ...
 */
function horseclub_adjacent_post_link_plus($args = '', $format = '%link &raquo;', $previous = true) {
  $defaults = array(
...
//  If Post Types Order plugin is installed, default to sorting on menu_order
  ...
  if ( $previous && is_attachment() ) {
    ...
    $posts = horseclub_get_adjacent_post_plus($r, $previous);
...
//  When num_results > 1, multiple adjacent posts may be returned. Use foreach to display each adjacent post.
  foreach ( $posts as $post ) {
    ...
    if ( empty($post->post_title) )
      $title = $previous ? esc_html__('Previous Post', 'horseclub') : esc_html__('Next Post', 'horseclub');
...
//    Truncate the link title to nearest whole word under the length specified.
    ...
    $rel = $previous ? 'prev' : 'next';
		$classm ='';
	$classm = $previous ? 'class="animsition-link"' : 'class="animsition-link"';
    $anchor = '<a href="'.get_permalink($post).'" rel="'.$rel.'"'.$tooltip.' '.$classm.'>';
